...
| **①'** | 名前は取れたが画像が無いとき | 画像だけ Item Search → 容量違いの兄弟SKU → Yahoo で補完（商品名は変えない） |
| ② | 楽天Item Search API | 製品ごと取れないときのフォールバック。画像あり出品を最大10件見て `r.r10s.jp` を優先 |
| ③ | Yahoo!ショッピングAPI | 最終フォールバック |

画像列だけ直すときは `npm run collect:img`（`--img-only`）。名前・説明は上書きしません。

# pet925 管理マニュアル

## 1. 運用ワークフロー（黄金のサイクル）
データの更新は必ず以下の順番で行ってください。

1.  **Geminiに依頼**: 商品情報をマニュアルの「列構成」通りに出力するよう依頼する。
2.  **マスターへ貼り付け**: Geminiの回答を `pet925_master.ods` にペーストし、タグなどを微調整する。
3.  **CSV書き出し**: [ファイル] > [保存コピーを保存] から `products.csv` という名前で保存。
4.  **自動変換**: ターミナルで `node convert.js --watch` を一度実行しておけば、保存のたびに自動で更新されます。
5.  **確認**: ブラウザで `index.html` を開き、**Ctrl + F5** で強制リロード。

---

## 2. 許可されているタグ一覧 (tags列)
フィルター機能を動かすために、以下の単語のみを使用してください（すべて半角英字）。
複数のタグを入れる場合は**半角スペース**で区切ります。

### 種類 (Animal)
- `dog` : 犬用
- `cat` : 猫用

### お悩み・特徴 (Condition)
- `tear` : 涙やけ
- `gf` : 穀物不使用 (Grain Free)
- `diet` : 体重管理
- `kidney` : 腎臓・尿路
- `senior` : シニア・高齢犬
- `adult` : 成犬用
- `skin` : 皮膚・被毛ケア
- `lamb` : ラム肉
- `venison` : 鹿肉
- `joint` : 関節ケア
- `tooth` : 歯垢・歯石ケア
- `appetite` : 食いつき・偏食

---

## 3. 注意事項（ここだけは気をつけて！）

### ① ファイル名と場所
- フォルダ内に `products.csv` という名前で存在する必要があります。
- `products (1).csv` などの余計なファイルがあれば削除してください。

### ② Linux（LibreOffice）の挙動
- `.~lock.products.csv#` というファイルは、編集ソフトが開いている間だけ出る「鍵」です。**無視して大丈夫**です。ソフトを閉じれば消えます。
- CSV保存時に「テキスト形式で保存しますか？」と聞かれたら **[はい]** を選んでください。

### ③ A8.netなどの広告コード
- `img` 列には「画像のURL」のみ、`a8` 列には「リンク先のURL」のみを貼り付けてください。
- コードを丸ごと貼り付けると表示が崩れる原因になります。
- **価格列 (amz_p, rak_p, yah_p)**:
    - **半角数字のみ**（カンマ不要）で入力してください。
    - 入力すると、システムが自動で比較し、最も安いショップに「最安値」バッジを表示します。

### ④ ブラウザのキャッシュ
- `node convert.js` を実行しても画面が変わらない場合、ほとんどがブラウザの記憶（キャッシュ）のせいです。迷わず **Ctrl + F5** を押しましょう。

---

## 4. LibreOffice Calc へのヒント
- **入力規則の設定**: [データ] > [入力規則] > [入力値の種類] で **「ユーザー定義」** を選択します。これが Google スプレッドシートの「カスタム数式」と同じ機能です。
- **ロックファイル**: `.~lock.products.csv#` は無視してOKです。
- **GitHubから復旧する場合**:
    1. GitHubの `products.csv` の内容をコピー。
    2. CalcのA1セルに貼り付け。
    3. 「テキストインポート」画面で、区切り記号を **「コンマ」**、テキスト区切りを **「"」** に設定する。
    4. 貼り付け後、再度 `.ods` 形式で保存する。

---

## 5. Gemini（AI）への依頼テンプレート
新しい商品を追加する際は、以下の構成をコピーしてGeminiに渡してください。

**依頼文の例：**
> 以下の商品の情報を、指定の列構成でCSV形式（コードブロック）で出力してください。
> タグはマニュアルの許可リストから選んでください。
> 
> 【列構成】
> `name,brand,tags,desc,img,amz,rak,yah,a8,label,promo`
> 【商品名】
> （ここに商品名やURLを貼る）

---

## 6. 開発者向けセットアップ（テストと自動化）
新しいパソコンで作業を始める場合は、まず Node.js をインストールしてから以下を実行：

0.  **Node.jsインストール**: `sudo apt install nodejs npm`
1.  **初期化**: `npm init -y` (package.jsonの作成)
2.  **ツール設置**: `npm install --save-dev jest` (テストツールの導入)
3.  **テスト実行**: `npx jest` (convert.test.jsの実行)
4.  **監視モード**: `node convert.js --watch`

---

## 7. 公開（デプロイ）手順
1.  GitHub にプロジェクトを push する。
2.  リポジトリの [Settings] > [Pages] を開く。
3.  Branch を `main` に設定して保存。数分後にURLが発行される。
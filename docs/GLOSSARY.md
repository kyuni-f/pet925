...
| `data_master.js` | タグ・カテゴリ・ブランド・店員コメントの設定。**ビルドが生成する。手編集しない。** |
...
| **モーダル** | 画面の上に被さる確認窓。お気に入り解除・画像拡大・利用規約。 |
...
| **難読化** | 問い合わせメールを文字コード配列にして、ソースを見たボットに拾われにくくする。完全な秘密ではない。 |
...
| `fetch(url)` | ネットからファイルを取る。Worker が `product_data_0.json` を読む。 |
...
| **GET** | 「ください」。検索やファイル取得。 |
| **POST** | 「受け取って処理してください」。Gemini に文章生成を頼むとき。 |

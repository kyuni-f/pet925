...
- `label` 画像左上バッジ / `promo` 説明文下のオレンジ文言（どちらも `#` なら非表示。絞り込みは未接続）
...
| **バッジ** | 商品画像左上の金色ラベル。CSV の `label`。`.featured-badge`。 |
| **プロモ文言** | カードの説明文の下に出るオレンジの一文。CSV の `promo`。`.promo-text`。cond タグ（カード下の小さなタグ）とは別。 |

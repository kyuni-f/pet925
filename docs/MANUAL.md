...
> **💡 キャッシュ回避**: `data_master.js` に埋め込まれた `siteVersion` が自動でキャッシュを制御します。通常は意識不要ですが、万が一反映されない場合は **Ctrl + F5**（強力な再読み込み）を行ってください。
...
Gemini に商品データ作成を依頼する際は、以下のルールを厳守してください。
...
### 5. システムへの影響
作成されたデータに不備（未定義タグ、商品名の重複、列数不足）がある場合、**Pythonのビルドシステムが検知し、デプロイ（公開）を自動停止します。** そのため、15列の構成と許可タグの整合性には細心の注意を払ってください。

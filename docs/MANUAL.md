...
5.  **確認**: プロジェクト直下の `index.html` をブラウザで開き、**Ctrl + F5**。
...
- **価格と最安値表示**:
    - 各ショップボタンが正しく表示され、それぞれのモールへリンクしているか？
    - ※価格表示および最安値バッジ機能は現在「実装予定」のステータスです。
- **リンクの動作**:
...
- **`data.js` の確認**: プロジェクト直下の `data.js` を開き、最新のデータが反映されているか？
...
### ⑤ エラーメッセージの改善と対応
- **詳細なヒント**: `convert.js` のエラーメッセージは、何が問題で、どうすれば解決できるか具体的な「👉 [解決策]」を提示するようになりました。エラー発生時は、ターミナルやGitHub Actionsのログを注意深く確認してください。
- **CI環境での挙動**: GitHub Actions上でエラーが発生した場合、`::error::` 形式でハイライト表示され、ビルドが自動的に停止します。これにより、不完全なデータでのデプロイを防ぎます。
- **主なエラーと解決策の例**:
    ...
    - **「出力先のフォルダが見つかりません」**: `convert.js` はプロジェクトのルートディレクトリ（`index.html` がある場所）で実行する必要があります。
    ...
    - **「ReferenceError」**: プログラム内の変数名が間違っている可能性があります。最近書き換えた箇所を確認してください。
...
3.  **ローカル環境での最終確認**
    - ターミナルで監視モード（`npm start`）が動いていることを確認します。
    - バリデーションエラー（⚠️マーク）が出ていないかターミナルのログをチェックします。
    - ブラウザで `index.html` を開き、新商品が正しく表示・検索できるか確認します。
...
4.  **GitHubへのデプロイ（公開実行）**
    - ターミナルで以下のコマンドを実行します：
      ...
      npm run deploy
      ```
    - これにより、データの保存・コミット・GitHubへのプッシュが一度に行われます。

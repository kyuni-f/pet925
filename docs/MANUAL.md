...
5. **GENERATE DESC** → 出てきた文を `desc` 列に貼る。`tags` 列は触らない（説明文の単語からタグは付かない）。
...
| `exclude_tags` | 任意。商品名の硬い一致提案だけを黙らせたい `key`。例: `appetite`。日常の主経路ではない |
...
**使われる場面は2つです（ビルド時に自動でタグを付け直すことはしません。説明文の単語でも提案しません）。**

1. `auto_collect_all.py` で商品を収集するとき、`GEMINI_API_KEY` が無い/AI呼び出しに失敗したときのフォールバック判定
2. `npm run build` 時、既にタグが付いている商品の検索を助ける読み・別名として（例: `digestive` タグが付いている商品は「おなか」でも検索にヒットする）。あわせて `build_report.html` の「商品名からの硬い一致」にも、商品名側のキーワードとして使う（説明文は見ない。タグは付けない）

タグそのもの（バッジ表示・フィルター）は常に `products.csv` の `tags` 列に書かれているものだけが使われます。説明文に「穀物不使用」などと書いてもタグは付きません。商品名にキーワードがあるのにタグが無い場合だけ、確認推奨が出ます。誤検知を黙らせたいときだけ `exclude_tags` へ。

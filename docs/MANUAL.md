...
### A. 効率的なデータ収集（推奨）
1.  **楽天で探す**: 楽天市場で登録したい商品の詳細ページを開く。
2.  **画像URL取得**: 登録した「画像抽出」ブックマークをクリックして高画質URLをコピー。
3.  **マスター反映**: `pet925_master.ods` の `img` 列（F列）に貼り付け。
4.  **仕上げ**: 商品名（A列）、ブランド名（B列）、タグ（C列）を手動で入力または調整。
    - ※商品名やURLもツールで一括取得したい場合は、15列版のブックマークレットも利用可能です。
...
> **💡 開発中の自動ビルド**: `npm start` を実行すると、CSVの変更を自動で検知して JSON を再生成します。ただしブラウザの表示を更新するには、ブラウザ側でのリロード（Ctrl + F5）が必要です。
...
### ③ A8.netなどの広告コード (img, amz, rak, yah, a8)
...
**💡 画像URLの取得コツ**:
1. 楽天の商品ページで専用のブックマークレットを使用すると、サイズ制限が解除された高画質なURLが取得できます。
2. 自動取得されたURLはサイズ制限が解除された高画質なものになります。
3. 手動で取得する場合は、URL末尾の `?_ex=...` を削ると画像が綺麗になります。

**⚖️ 画像利用に関するガイドライン**:
- 商品紹介（引用）の範囲内であれば、ショップへのリンクを併記することで実務上許容されます。
- 楽天等の画像URLを直接指定する方式（インラインリンク）を採用しており、自サーバーへの複製を避けることで著作権リスクを低減しています。
- **Amazonの画像について**: Amazonの画像URLは有効期限や技術的保護が厳しく、リンク切れのリスクが高いため、原則として**「楽天の商品ページ」から取得した画像URLを使用すること**を推奨します。
- アフィリエイト提携後は各プラットフォームの規約により、公式に画像利用が認められる状態となります。
...
### B. 商品データの作成依頼
商品データを AI に作成させる際は、**`docs/AI_INSTRUCTIONS.md`** の内容をそのままプロンプトとして渡してください。これにより、15列のCSV形式とタグの整合性が保たれます。

---

## 8. マスターファイルの一元管理（LibreOffice / Sheets）
...
## 10. 公開前チェックリスト (Pre-flight Check)
...
- **「GA4で30分間のデータ（リアルタイム）しか見えない」**: 標準レポートへの反映には24-48時間の遅延があります。また、詳細な検索ワードを見るには、GA4管理画面の「カスタム定義」で `item_label` をカスタムディメンションとして登録する必要があります。

## 12. GA4 の推奨設定
1. **データ保持期間の変更**: 「管理 > データの収集と修正 > データの保持」を **14ヶ月** に変更してください（デフォルトは2ヶ月で、古いデータが消えてしまいます）。
2. **カスタムディメンションの登録**: 「管理 > データの表示 > カスタム定義」から、パラメータ名 `item_label` を登録してください。これにより `search_no_results` 等で送信した具体的な単語がレポートに表示されます。
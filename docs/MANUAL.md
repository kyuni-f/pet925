...
2.  **画像URL取得**: 登録した「画像URL取得」ブックマークをクリックして高画質URLをコピー（末尾のサイズ制限 `?_ex=...` が自動で削られます）。
...
4.  **仕上げ**: 商品名（A列）、ブランド名（B列）、タグ（C列）を手動で入力または調整。
    ...
    - **JANコード**: F列に入力します。画像が空の場合、一部のショップ（petline等）の倉庫から自動取得を試みますが、失敗した場合は「No Image」が表示されます。
...
2. **重要**: ショップによって保存フォルダが異なるため、JANコードによる画像自動生成は「一部のショップ」でしか機能しません。
3. 画像が「No Image」になる、または表示されない場合は、迷わずブックマークレットを使って `img` 列にURLを直接貼り付けてください。
...
## 9. マスターファイルの一元管理（LibreOffice / Sheets）

...
#### 💡 補足: `jan_data_collector.py` との違い
`jan_data_collector.py` は画像・商品名・リンクのみを取得する簡易版です（説明文の自動生成やタグ判定は行いません）。基本的には機能が上位互換の `auto_collect_all.py`（`npm run collect` / `npm run collect:all`）の利用を推奨します。
